...
import com.example.proyecto_final2.CalendarViewActivity;
import com.example.proyecto_final2.R;

//creamos una clase que herede de BroadcastReceiver y sobrescribimos el método onReceive()
public class AlarmRec extends BroadcastReceiver {
    @Override
    //método que se ejectua cada vez que se produzca el evento
    public void onReceive(Context context, Intent intent) {
        String event = intent.getStringExtra("event");
        String time = intent.getStringExtra("time");
        int notId = intent.getIntExtra("id",0);
        Intent activityIntent = new Intent(context, CalendarViewActivity.class);
